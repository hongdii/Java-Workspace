...
public class ListRun {
	...
	public static void main(String[] args) {
		 ...
		 * 	== 컬렉션
		 * 	=> 다량의 데이터들을 관리하고자 할때 배열 가지고 충분히 사용했었음, 단 배열의 단점들을 보완한 것이 컬렉션
		 * 
		 * 	배열과 컬렉션의 차이점
		 * 	- 배열의 단점
		 ...
		 * 	- 컬렉션의 장점
		 * 	1. 크기에 제약이 없다
		 * 		=> 크기 지정을 해줄 필요도 없고 만일 크기 지정을 해놔도 알아서 크기가 늘어나면서 새로운 데이터를 추가해주는
		 * 			코드가 이미 정의되어 있음
		 * 	2. 중간에 값을 추가하거나 삭제하는 경우 값을 땡겨주는 코드가 이미 메소드로 정의되어있음
		 * 	3. 기본적으로 여러 타입의 데이터들을 저장할 수 있음(단, 제너릭 설정을 통해서 한 타입의 데이터들만 들어올수도 있음) 다형성이 있기때문
		 * 
		 ...
		 *  
		 *  Set 계열 => 담고자 하는 값(value)만 저장 / 값 저장시 순서유지x(index개념이 없음) / 중복값 허용x
		 *  			ex) HashSet, TreeSet 
		 *  
		 *  Map 계열 => 키(Key) + 담고자 하는 값(value)세트로 저장 / 값 저장시 순서유지x(index개념이 없음)
		 *  			/ key 중복 허용x
		 ...
		 * 2. 컬렉션에 저장된 객체를 꺼내서 사용할때 매번 형변환하는 절차를 없애기 위해서.
		 * 
		 */
		//1. add(E e) : 해당 리스트의 끝에 전달된 e를 추가시켜주는 메서드.
		list.add(new Music("Cookie", "new jeans"));
		list.add(new Music("12월의 기적", "exo"));
		list.add(new Music("die for you", "the weekends"));
		//list.add("ㅎ_ㅎ"); // 제네릭으로 Music객체 선언해줘서 다른 객체 사용못함
		// 되도록 제네릭 설정해주는것이 좋음
		/*
		 * 순서가 유지 되면서 값 추가됨.
		 * 각 index에 담겨있음.
		...
		// 3. set(int index, E e) : 리스트에 원하는 위치의 인덱스 값을 제시하면 전달하는 e값으로 변경해주는 메서드
		list.set(0, new Music("피 땀 눈물", "BTS"));
		
		System.out.println(list);
		
		// 4. remove(int index) : 리스트에서 해당 인덱스 위치에 있는 값을 삭제시켜주는 메서드.
		list.remove(1);	// 값을 삭제해주면 배열에 빈공간이 생기는 것이 아니라, 값을 삭제하고 뒤에 있는 값을 땡겨옴
...
//		
		// 제네릭 설정 후
		Music m = list.get(0);	// 반환되는 값도 Music타입임
		...
		// 7. subList(int index1, int index2) : 해당 리스트로부터 index1에서 index2까지의 데이터 값들을 추출해서 새로운 List로 변환시켜주는 메서드
		...
		// 9. isEmpty() : 해당 리스트가 비어있는지 묻는 메서드(비어있으면 true/ 비어있지 않으면 false 반환)
