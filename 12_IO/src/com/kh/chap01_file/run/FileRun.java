...
public class FileRun {
	...
	public static void main(String[] args) {
		...
		try {
			...
			File file1 = new File("test.txt");	// 상대경로
			file1.createNewFile();
			
			// 2. 경로 지정을 한 파일명(C:\\aaa\\test.txt)을 제시해서 생성하기 => 이 때 존재하는 경로를 제시해야함.
			File file2 = new File("C:\\aaa\\test.txt");		// 절대경로
			file2.createNewFile();
			...
			// File에서 제공하는 메소드들.
			System.out.println(folder.isFile());	// isXXX() : XXX입니까? 를 의미하는 메서드
		 ...
		 * 		보조스트림 : 기반스트림으로 부족한 성능을 향상시켜주는 용도의 스트림(직접적으로 외부매체와 연결할 수 없음, 즉 단독사용이 불가)
		 * 				  (보조해주는 기능 : 속도향상, 자료형 변환, 객체단위의 입출력 보조 등등)
		 * 				  단, 보조스트림은 단독적으로 사용이 불가능하므로 반드시 외부매체와 직접적으로 연결 가능한 기반스트림과 함께 사용해야한다.
		 * 
		 */
	}
