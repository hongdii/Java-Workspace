...
public class Run {
	...
	public static void main(String[] args) {
		...
		System.out.println(Thread.currentThread().getName()+"의 우선순위"
		...
		// 메모리값이 부족해졌을때 설정한 우선순위를 통해 실행
		// setPriority() : 우선순위를 지정해주는 메소드
		t1.setPriority(Thread.MIN_PRIORITY); // Thread.MIN_PRIORITY : 가장 작은 우선순위. 상수필드	// 1
		t3.setPriority(Thread.MAX_PRIORITY); // Thread.MAX_PRIORITY : 가장 큰 우선순위. 상수필드  // 10
		 ...
		 * 주의점은 스레드중 한개라도 setDaemon으로 설정시 모든 스레드에 setDaeon 설정을 해줘야함.
		 */
		System.out.println("메인 스레드의 끝부분");
		// 시작 순서는 우선순위와 다를 수 있지만 종료 순서는 우선순위(priority)가 높은 순으로 끝나게됨.(항상 그렇지 않음)
	}
