...
public class MainFrame extends javax.swing.JFrame {
	...
	public MainFrame() {
		...
		// plusBtn 클릭시 => 두 개의 정수값을 가져온 후 덧셈연산 결과를 result텍스트 상자에 출력
		plusBtn.addActionListener(new ActionListener() {
			// ActionListener는 actionPerformed를 갖기때문에 오버라이드해줌
			@Override
			public void actionPerformed(ActionEvent e) {
				// plusBtn 클릭시 실행할 내용
				int value1 = Integer.parseInt(num1.getText());
				int value2 = Integer.parseInt(num2.getText());
				result.setText( value1+value2+" " ); // setText()는 문자열 매개변수를 받기때문에 ""추가
			}
		});
		// ActionListener : 인터페이스이기때문에 곧바로 생성 불가.
		// 단, 여기서 생성을 하고 싶다면 "미완성된 메소드"를 완성시키면서 생성 => 익명 클래스 방식
		
		// minusBtn 클릭 시 => 두개의 정수값을 가져온 후 뺄셈연산 결과를 result텍스트박스에 출력
