...
public class C_Cast {
	 ...
	 * 
	 * 2. 같은 자료형끼리만 연산 가능.
	 * 	=> 즉, 다른 자료형끼리 연산을 수행하고 싶으면 한구문은 "형변환"을 해야함.
	 * [표현법] 값 + (바꿀자료형) 값;
	 * 
	 * 형변환의 종류
	 * 1. (암시적)자동형변환 => 자동으로 형변환이 진행되기 때문에 내가 직접 형변환할 필요가 없음.
	...
	public void autoCasting() {
		...
		int i1 = 10;
		double d1 = (double) i1; // 자동으로 형변환됨.
								 // 컴퓨터 포장용상자에 휴대폰을 담은꼴과 비슷함.
		...
		double result = /*(double)*/ i2+d2; // 12.0+3.30 => 15.3
		...
		long l4 = 2000; // L을 붙이지 않아도 문제가 없는 이유 : 자동형변환이 되기때문
		
		//3. 특이케이스 long(8byte) -> float(4byte)
		// 정수가 실수로 담길때에는 큰사이즈의 정수여도 작은 사이즈의 실수 변수에 대입 가능.
		//아무리 4byte float라고 해도 long형보다 담을 수 있는 값의 범위가 더 크기 때문에
		//float형이 실수이기때문에 long형보다 표현가능한 범위의 수가 더 커서 자동형변환이 이루어짐
		
		long l5 = 100000000L;
		float f5 = /*(float)*/15;
		System.out.println("f5 : "+f5);
	
		// 4. 특이테이스 char(2byte) <-> int(4byte) : 양방향 형변환 가능.
		...
		/*
		 * char의 범위 0~65xxx : 각 문자마다 고유의 정수값이 정해져있다. 따라서 쌍방향 변환이 가능. 음수값은 오류가 난다.
	...
	public void forceCasting() {
			...
			//iNumd이 double형으로 우선 변환이 됨. 10 -> 10.0
			// 10.0+15.89 실수값이 반환됨.
			...
			//			자동형변환은 작은 바이트변수가 큰 바이트 변수로 형변환되기 때문에
