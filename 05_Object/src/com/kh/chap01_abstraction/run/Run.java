...
public class Run {
	 ...
	 * 
	 * 	예) 학생관련 프로그램
	 * 		1) 학생 관련 객체(홍길동 학생, 민경민 학생, 김갑생 학생, 등등)
	 * 		2) 공통적인 속성 및 기능들(이름,나이,성별,주소,전화번호,키,달리기속도,학년,반,국어점수,영어점수,몸무게,자기,먹기,생각하기,달리기 등)
	 ...
	 * 				홍길동이라는 학생 객체를 만들기 위해선 => String name2 = "민경민", int age1 = 30; double height1 = 188.8;
	 * 											 => String name3 = ...;
	 * 				=> 관리하고자하는 학생들이 늘어나면 늘어날수록 생성해야할 갯수도 엄청나게 증가하게됨. 즉, 관리하기가 굉장히 힘듬.
	 * 
	 ...
	 * 				=> 변수를 사용하는 것보다는 나아보임.
	 * 				=> 하지만 관리하는 학생중 한명이 전학을 가게되는 경우 해당 인덱스에 있는 모든 키, 나이에 대한 정보도 제거를 해줘야함.
	 * 					그리고 새로운 학생이 들어오게 되는 경우 배열의 특성상 한변 크기를 지정하게되면 변경하는게 불가능하므로, 새로운
	 * 					정보를 입력하기위해 배열을 복사해서 새롭게 추가해야함. 즉, 유지보수가 힘듬.
	 * 
	 * 			3) "클래스(구조체)"를 가지고 프로그래밍한다면
	 * 				구조제 : 한번에 여러개의 자료형을 보관할 수 있는 배열같은 형태
	...
	public static void main(String[] args) {
		...
		hong.name = "홍길동";
		hong.age = 20;
		hong.height = 166.7;	// 필드 접근하고자한다면 .을 통해 가져올 수 있음(단, 접근제한자가 public인 경우)
		 ...
		 * => 외부에서 함부로 값을 변질시키거나 조회권환이 없는 사람이 값을 쉽게 가져올 수 있는 문제 발생
		 * >>> 캡슐화로 해결
		 * 객체지향의 설계 원천중 하나가 바로 정보 은닉이며 그 기술 중 하나가 캡슐화
		 * 
		 */
	}
