...
public class Run {
	 ...
	 * 				클래스에서의 가장 중요한 목적인 '데이터 접근권한'을 원칙으로
	 * 				외부로부터 '데이터의 접근을 막고'
	 * 				대신에 '데이터를 간접적으로나마 처리할 수 있는 메소드를 클래스 내부에 작성해서 관리하는 방식.
	 * 	캡슐화를 하지않으면 : 외부로부터 직접접근이 가능하기 때문에 함부로 값이 변질되거나 조회를 막지 못하는 문제 발생
	 * 	
	 * 	1) 정보은닉 : private
	 ...
	 * 
	 * 	2) setter/getter 메소드
	 * 		간접적으로나마 접근해서 값을 담거나, 그값을 가져올 수 있는 메소드 setter/getter 메소드
