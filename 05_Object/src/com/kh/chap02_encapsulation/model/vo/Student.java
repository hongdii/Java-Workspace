...
public class Student {
	 ...
	 * }
	 */
	//setter  -> setXXX : setter 메서드는 보통 이름을 setXXX로 지어줌
	// 해당 필드에 대입하고자하는 값을 전달받아 해당필드에 대입시켜주는 기능의 메소드
	// 이때, 이 메소드는 접근 가능하도록 해야하기 때문에 public 접근제한자를 사용
	
	// 이름값을 기록 및 수정할 수 있는 기능의 메소드(name 필드에 값을 넣기 위한 용도)
	// set+필드명 => 두 단어를 이어줄때 카멀케이스(낙타등표기법)활용하여 메서드명 선언
	public void setName(String name) {
		//매개변수 : 해당 메소드 호출시 전달되는 값을 받아주기 위한 변수
		
		// name = name
		// 매개변수name = 매개변수name을 대입하는 형식임.
		// 왜? 필드명과 매개변수명이 동일한 경우 {}영역 내에서는 해당영역에서 만들어진 변수가 좀더 우선순위가 높기때문.
	...
	// 키값을 기록 및 수정할 수 있는 기능의 메소드(heught필드에 값을 대입하기위한 용도)
	...
	public String information() {
		//xxx님의 나이는 xx살이고 xx입니다.
		return name+"님의 나이는 "+age+"살이고 "+height+"입니다.";
	}
