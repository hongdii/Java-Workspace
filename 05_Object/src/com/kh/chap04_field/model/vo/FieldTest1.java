...
/*
 * 변수구분
 * -전역변수 : 클래스 영역에 바로 전언하는 변수 -> 클래스 내에서면 어디서든 사용 가능
 * -지역변수 : 클래스 영역 내의 특정 영역({})에 선언한 변수(선언한 영역에서만 사용 가능)
 * 			ex) 메소드영역만, if문, for문 등등
 * 
 * 1. 전역변수(2가지로 구분됨)
 *  - 멤버변수 == 인스턴스 변수 == 필드
 ...
 * 		소멸시점 : 객체 소멸시 같이 소멸
 * 
 *  - 클래스변수 == static변수 : static이라는 예약어가 붙어있냐 아니냐에 따라 멤버변수나 클래스변수냐로 구분됨.
...
public class FieldTest1 {	// FieldTest1 클래스 시작
	...
	public void test(int num) {	 // test 메서드 시작
		...
		/*
		 * 멤버변수 출력시 에러가 안나는 이유
		 * 해당 클래스 객체 생성시 jvm이 값을 해당필드로 자동으로 대입(초기화)시켜 줬기 때문
		 * 물론, 명시적 초기화도 가능함.
		 */
		System.out.println(global);
		
		// 매개변수 출력시 에러 안나는 이유
		// 해당 메서드(test)호출시 반드시 값을 넘겨줘야 호출이 가능하기때문에 num에는 반드시 어떤값이든 초기화가 될 것.
