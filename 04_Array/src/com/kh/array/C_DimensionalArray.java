...
public class C_DimensionalArray {
	...
	public void method1() {
		 ...
		 * 안쪽 for문 => 열에 대해 지정(0열 ~ 4행) == (0열~ 각 행별 열의 크기 -1)
	...
	public void method3() {
		...
		for(int i=0 ; i<arr2.length ; i++) {
			for(int j=0 ; j<arr2[0].length ; j++) {
				System.out.print(arr2[i][j]+"\t");	// \t : tap을 의미.
			}
			System.out.println();
		}
	}
	...
	public void method4() {
		 ...
		 * 즉, 묶여있는 일차원배열의 길이가 꼭 같을 필요가 없다.
		 * 
		 * 단, 행크기는 생략이 불가, 반드시 지정해야함.
		 * 열 크기는 생략이 가능하다. 각행의 열의 길이가 변경 가능함.
		 * 0
		 */
		int[][] arr = new int[3][];		// 가변 배열(행 크기 3행)
	...
	public void method5() {
		...
		char value = 'a'; // 아스키코드표에서 98 , 따라서 a+1 = b
 		for(int i=0 ; i<arr.length ; i++) {
			for(int j=0 ; j<arr[i].length ; j++) {
				arr[i][j] = value;
				
				value = (char)(value + 1);	// value가 int값으로 변환되기때문에 char로 변환해주기
				System.out.print(arr[i][j]+"\t");
			}
			System.out.println();
		}
