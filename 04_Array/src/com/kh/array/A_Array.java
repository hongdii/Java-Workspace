...
public class A_Array {
	...
	public void method1() {
		...
		int [] arr = new int[5];	// 배열은 값이 들어가있지 않으면 안됨. 
									//따라서 배열 자료형이 int형일때 디폴트로 0의 값이 저장되어있음
		int a = 0;
		...
		// 각각의 인덱스 값을 확인하기 위해서는 []블럭을 통해 내가 확인하고자하는 인덱스(방번호)를 제시해줘야함.
		// 그럼 arr에 전체 담겨있는 값을 확인하고싶다면?
		System.out.println(arr); 	// 콘솔창 출력값은 전체 담겨있는 값이 나오는 것이 아닌, 배열이 위치한 주소를 출력해줌.
									// 실제 arr이라는 값에는 주소값이 담겨있음
		// [I@1888ff2c 여기서 @기준으로 우측은 참조하고있는 heap 영역의 주소값
		// [ : 배열임을 나타냄
		// I : int 자료형을 의미함.
	}
	...
	public void method2() {
		...
		System.out.println("arr의 해시코드 값 : "+arr.hashCode());		// 해시코드값은 고유하기 때문에 배열마다 값이 다름.
		 ...
		 * => 실제 값(리터럴)을 바로 stack영여에서 담을 수 있는 변수 => 일반변수
		 * 
	...
	public void method5() {
		...
		//int i2 = null;		Type mismatch -> null 값을 int 자료형에 넣을 수 없음.
		int[] arr = null;	// null값을 할당 가능. 주소값을 담을 수 있는 변수의 기본값은 null.
	...
	public void method6() {
		...
		// arr을 새롭게 만들어줬기 때문에 이전 arr에 저장된 값들은 heap영역에 떠다니게됨.
		// 떠다니는 값이 계속 사용되지않으면 자동으로 삭제됨.
		...
		/*
		 * 항상 고유한 주소값 부여, 기존에 생성되었던 주소와 절대로 겹치지않는다
	...
	public void method7() {
		...
		// 방법2.
		int[] arr2 = {1, 2, 3, 4};	// new int[]를 없애 간략화
	...
	public void method8() {
		...
		// 3. 반복문 활용해서 0번 인덱스부터 마지막 인덱스까지 배열에 담긴값 출력
		// 출력예시) arr[x] : xx
		
		int[] arr = new int[10];
		
		for(int i=0 ; i<arr.length ; i++) {
			arr[i] = (int)(Math.random()*100+1);	// Math.random()*범위끝수*범위첫수
			System.out.printf("arr[%d] : %d\n", i, arr[i]);
		}	
	}
	...
	public void method10() {
		// 사용자에게 문자열을 하나 입력받은후
		// 각각의 문자들을 뽑아서 char 배열에 옮겨담기
		
		// 1. 사용자에게 문자열 입력받기
		// 2. char 배열 생성해두기(배열 크기 == 문자열의 길이 만큼)
		// 3. 문자열에서 각 인덱스별로 문자를 뽑아서 char 배열의 각 인덱스에 담기
		// arr[0] = 문자열.charAt();
	...
	public void method11() {
		...
		System.out.println("짝수 값의 총합 : "+sum);
	}
	...
	public void method12() {
		...
		//3. 3명의 키의 총합을 출력하고, 3명키의 평균도 출력 (단, 소숫점 첫째자리까지 출력)
		...
		System.out.printf("3명 키의 평균 : %.1f",sum/3);
