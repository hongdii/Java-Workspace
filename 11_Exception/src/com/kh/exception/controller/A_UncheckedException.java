...
public class A_UncheckedException {
	 ...
	 * - ArrayIndexOutOfBoundsException : 배열의 부적절한 인덱스로 접근할때 발생하는 에러
	 * - NegativeaArraySizeException : 배열의 크기를 음수로 지정할 경우 발생하는 예외 new int[-5];	: -5
	 ...
	 * - .....
	 * 
	 * => 이러한 RuntimeException과 관련한 예외는 충분히 예상 가능한 상황이기때문에
...
//			System.out.println("0으로 나눌 수 없습니다.");
			e.printStackTrace(); 	// 현재 발생한 오류를 추적할 수 있는 로그정보제공
		}
		System.out.println("프로그램 종료.");	// 프로그램 종료룰 막고자 예외처리가 있는것.
	}
	...
	public void method2() {
		...
		try {
			...
			System.out.println("나눗셈 연산 결과 : " + (10/num)); // 예외 발생할수있음
		} catch(ArithmeticException e) {
			System.out.println("0으로 나눌 수 없습니다.");
		} catch(InputMismatchException e) {
			System.out.println("정수를 제대로 입력해주세요.");
		}	// 다중 catch 블럭으로 사용 가능 : 예외가 여러개 발생할 가능성이 있는 경우
	...
	public void method3() {
		...
		int size = Integer.parseInt(sc.nextLine());	// 어떠한 값을 입력해도 정수로 받음
		
//		if(size > 0) {	// 방법 1. if문으로 size 값이 양수값일때만 생성하도록 제어
//			int[] arr = new int[Math.abs(size)];	// 방법 2. 항상 양의 정수값이 들어갈 수 있도록 abs메서드 사용
//		}
...
//			System.out.println("예외가 발생하긴 했네요. 근데 배열의 크기를 음수로 입력했거나, "
//					+ "부적절한 인덱스로 접근한 것 같습니다.");
//		}	// 다중 catch블럭이 줄어들었음. 정확히 어떤 에러인지 알수없음.
		...
		try {
			...
			System.out.println("배열의 크기를 음수로 지정할수 없습니다.");
		} catch(RuntimeException e) {	// 다형성을 적용하여 부모타입의 예외클래스 작성 가능.
			System.out.println("예외가 발생하긴 했네요. 근데 배열의 크기를 음수로 입력했거나, "
					+ "부적절한 인덱스로 접근한 것 같습니다.");
		} // 다중 catch에서는 블럭의 순서가 중요함. => 범위가 작은 자식타입의 예외 클래스를 먼저 기술해야함.
	}
	
	/*
	 * RuntimeException 관련된 예외는
	 * - 조건문으로 해결 가능 => 예외 자체가 발생 안되게끔 개발자가 소스코드로 핸들링 가능
	 * - 예외처리 구문으로도 해결 가능 => 예외가 발생했을때를 대비해서 그때 실행할 내용을 정의해두는 것
	 * 
	 ...
	 * 
	 * UncheckedException -> 필수로 체크하지는 않아도 되는 예외라는뜻
	 * 
	 */
}
